...
1. A new exported pure helper `evaluateAutonomousRunQualityGate(runId)` fetches quality gates and returns a `AutonomousRunQualityGateDecision`.
...
| 7 | Deep block: independentReview.status `failed`/`blocked`, independentReview.blockingIssueCount > 0, resultStateConsistency.status `inconsistent`, resultStateConsistency.blockingIssueCount > 0, or contractCoverage.conflictCount > 0 must block even if top-level status is permissive. | Six separate tests cover each deep block condition; each asserts `blocked: true, workerResult: null`, executor not called. (Note: store normalization already propagates these to the top-level status; tests verify both the stored normalization and the checker's independent deep evaluation.) | covered |
...
| 11 | No Chrome/background/runtime files touched. | `git diff --name-only` shows only `core/run/orchestrator.ts`, `tests/run-orchestrator.test.ts`, `tests/run-result-consistency.test.ts`. | covered |
| 12 | No new dependencies. | `package.json` unchanged; implementation uses only existing types and store functions. | covered |
