...
| Failed, blocked, and running lanes produce attention states but keep `canProceed: true`. | `createPetReviewLaneGate marks failed, blocked, and running lanes as attention without blocking progress` |
...
- `attention / failed_lane`, `attention / blocked_lane`, or `attention / active_review` for non-blocking review attention.
- `clear / none` when no review lane requires attention.

`blockingLaneCount` only counts lanes that carry P1, P2, or `block` recommendation when the gate is blocked. It is zero for attention and clear gates.

The handoff capsule treats `reviewLaneSummaries` as the source for projected review-lane counts and gate fields. Caller-provided aggregate counts or gate fields are not trusted when the capsule is built.
