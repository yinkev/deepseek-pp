...
`blockingLaneCount` only counts lanes that carry P1, P2, or `block` recommendation when the gate is blocked. It is zero for attention and clear gates. Gate derivation uses all sanitized review lanes; handoff may expose only the first four summaries, but hidden blocking lanes still affect the gate/count projection.
