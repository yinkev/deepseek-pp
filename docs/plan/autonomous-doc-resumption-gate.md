...
The structured contract block below is the machine-readable source of truth for this gate. Prose can explain history and review iterations, but it does not revoke the contract. To revoke or change the posture, update `contract_status` or remove the relevant marker line.
...
| Prose after the structured block cannot silently revoke the machine contract. | `treats the exact current structured contract as authoritative over later prose`. |
...
Results: focused suite passed 34/34; TypeScript compile passed; full suite passed 873/873; diff check passed; `entrypoints/background.ts` diff was empty.
...
- The structured contract is intentionally authoritative over later prose; revocation must change `contract_status` or remove marker lines.
