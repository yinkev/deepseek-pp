...
import {
  decodeProviderRuntimePayload,
  type ProviderRuntimeDecodedPayload,
  type ProviderRuntimePayloadCommandType,
...
type ProviderTypedCommandType = ProviderRuntimePayloadCommandType & TypedRuntimeCommandType;
...
export function defineProviderPayloadRuntimeCommandHandler<
  TType extends ProviderTypedCommandType,
>(
  type: TType,
  handle: (
    payload: ProviderRuntimeDecodedPayload<TType>,
    context: RuntimeMessageContext,
  ) => MaybePromise<TypedRuntimeCommandResponse<TType>>,
): RuntimeCommandHandler<TType> {
  return defineRuntimeCommandHandler<TType, ProviderRuntimeDecodedPayload<TType>>({
    type,
    decode(message) {
      const payload = Object.hasOwn(message, 'payload') ? message.payload : undefined;
      return decodeProviderRuntimePayload(type, payload);
    },
    handle,
  });
}
