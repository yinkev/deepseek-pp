...
import { createSidepanelLegacyToolStream } from '../../core/chat/sidepanel-legacy-tool-stream';
...
export function createChatRuntimeService(
  ...
  const runWebToolLoop = async (
    ...
    for (let step = 0; step < MAX_CHAT_TOOL_STEPS; step++) {
      ...
      const stream = createSidepanelLegacyToolStream(
        toolDescriptors,
        (delta) => emitChunk(turn, { text: delta, done: false }, excludeTabId),
      );
      ...
      const result = await dependencies.submitWebPrompt({
        ...
        onTextChunk: stream.onTextChunk,
      }, turn.controller.signal);
      assertTurnActive(turn);
      stream.finishStream();

      chatParentMessageId = result.responseMessageId;
      const fullText = stream.getFullText(result.assistantText);
      if (!fullText) {
  ...
  const runOfficialToolLoop = async (
    ...
    for (let step = 0; step < MAX_CHAT_TOOL_STEPS; step++) {
      ...
      const stream = createSidepanelLegacyToolStream(
        ...
        (delta) => emitChunk(
          turn,
          { text: delta, done: false, phase: 'answer' },
          excludeTabId,
        ),
      );
  ...
  const submitPrompt: ChatRuntimeService['submitPrompt'] = async (request, excludeTabId) => {
    ...
    const admission: PendingChatAdmission = { generation, admissionLease };
  ...
  const runImageUpload = async (
    ...
    try {
      ...
      if (controller.signal.aborted) assertSignalActive(controller.signal);
      throw error;
    }
