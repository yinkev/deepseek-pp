...
import { createProviderChatTurnCoordinator } from '../entrypoints/background/provider-chat-turn-coordinator';
import {
...
describe('R4.3 DeepSeek runtime ownership', () => {
  it('creates exactly the assigned 16 typed handlers and eight receiving decoders', () => {
    const handlers = createDeepSeekRuntimeHandlers({
      ...
      chat: {
        ...
        submitProvider: vi.fn(async () => ({ ok: true as const })),
  ...
  it('routes provider and legacy chat submissions inside the typed handler', async () => {
    const legacySubmit = vi.fn(async () => ({ ok: true as const }));
    const providerSubmit = vi.fn(async () => ({ ok: true as const }));
    const service: ChatRuntimeService = {
      submitPrompt: legacySubmit,
      uploadImage: vi.fn(async () => ({ ok: false as const, error: 'unused' })),
      resetSession: vi.fn(async () => undefined),
      reconcileInterruptedOnWake: vi.fn(async () => undefined),
    };
    const handlers = createDeepSeekRuntimeHandlers({
      auth: createAuthDependencies(),
      multimodal: createMultimodalDependencies(),
    ...
    await expect(dispatch(handlers, {
      ...
      payload: {
        ...
        model: { providerId: 'qwen-web', modelId: 'stale' },
        logicalConversationId: 'conversation-1',
      },
    ...
    await expect(dispatch(handlers, {
      type: 'CHAT_SUBMIT_PROMPT',
      payload: { text: 'legacy', refFileIds: [] },
    })).resolves.toEqual({ ok: true });
    expect(legacySubmit).toHaveBeenCalledWith({
      text: 'legacy',
      refFileIds: [],
    }, 17);
  });
...
describe('interactive chat coordinator', () => {
  ...
  it('shares admission between provider-backed and legacy turns', async () => {
    const admissionCoordinator = createChatAdmissionCoordinator();
    const providerTurn = deferred<void>();
    const provider = createProviderChatTurnCoordinator({
      admissionCoordinator,
      getChatEnabled: vi.fn(async () => true),
      runTurn: vi.fn(() => providerTurn.promise),
      handleTurnError: vi.fn(),
      resetState: vi.fn(),
    });
    const legacyCompletion = deferred<ModelTurn>();
    const dependencies = createChatDependencies();
    dependencies.admissionCoordinator = admissionCoordinator;
    vi.mocked(dependencies.submitWebPrompt).mockReturnValue(legacyCompletion.promise);
    const legacy = createChatRuntimeService(dependencies);

    await expect(provider.submit({ text: 'provider' })).resolves.toEqual({ ok: true });
    await expect(legacy.submitPrompt({ text: 'legacy blocked', refFileIds: [] }))
      .resolves.toEqual({ ok: false, error: 'chat_already_running' });

    providerTurn.resolve();
    await provider.resetSession();
    await expect(legacy.submitPrompt({ text: 'legacy', refFileIds: [] }))
      .resolves.toEqual({ ok: true });
    await waitForCall(dependencies.submitWebPrompt);
    await expect(provider.submit({ text: 'provider blocked' }))
      .resolves.toEqual({ ok: false, error: 'chat_already_running' });

    legacyCompletion.resolve(modelTurn('done', 101));
    await waitForCall(dependencies.markChatLoopFinished);
  });
  ...
  it('filters streamed tool markup and completes without duplicating visible text', async () => {
    const dependencies = createChatDependencies();
    vi.mocked(dependencies.buildPrompt).mockResolvedValue({
      augmented: 'use tool',
      enabledDescriptors: [toolDescriptor()],
    ...
    vi.mocked(dependencies.submitWebPrompt)
      .mockImplementationOnce(async (_input, callbacks) => {
        const fullText = 'Visible <sample_tool>{"value":"once"}</sample_tool>';
        callbacks.onTextChunk?.('Visible ', 'Visible ');
        callbacks.onTextChunk?.(
          '<sample_tool>{"value":"once"}</sample_tool>',
          fullText,
        );
        return modelTurn(fullText, 101);
      })
      .mockImplementationOnce(async (_input, callbacks) => {
        callbacks.onTextChunk?.('Final answer', 'Final answer');
        return modelTurn('Final answer', 102);
      });
    vi.mocked(dependencies.executeToolCall).mockResolvedValue({
      ok: true,
      summary: 'done',
      detail: 'done',
      name: 'sample_tool',
    ...
    await expect(service.submitPrompt({ text: 'use tool', refFileIds: [] }, 17))
      .resolves.toEqual({ ok: true });
    await waitForCall(dependencies.markChatLoopFinished);

    const chunks = vi.mocked(dependencies.broadcastChunk).mock.calls
      .map(([chunk]) => chunk);
    expect(chunks.map((chunk) => chunk.text).join('')).not.toContain('<sample_tool>');
    expect(chunks.at(-1)).toEqual({ text: '', done: true });
    expect(chunks.filter((chunk) => chunk.text === 'Final answer')).toHaveLength(1);
  });
  ...
  it('suppresses stale image gate rejection after reset aborts the upload', async () => {
    const gate = deferred<boolean>();
    const dependencies = createChatDependencies();
    vi.mocked(dependencies.getChatEnabled).mockReturnValue(gate.promise);
    const service = createChatRuntimeService(dependencies);
