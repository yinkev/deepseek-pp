...
describe('fetch hook request lifecycle', () => {
  ...
  it('captures fetch credentials only for exact DeepSeek API origins', async () => {
    const nativeFetch = window.fetch;
    const fetchImpl = vi.fn(async () => new Response(null, { status: 204 }));
    window.fetch = fetchImpl;
    ...
    try {
      uninstall = hookFetch();
      await window.fetch('https://chat.deepseek.com/api/v0/users/current', {
        headers: { Authorization: 'Bearer deepseek' },
      });
      await window.fetch('https://evil.example/api/v0/users/current', {
        headers: { Authorization: 'Bearer foreign-path' },
      });
      await window.fetch('https://evil.example/collect?next=https://chat.deepseek.com/api/v0/users/current', {
        headers: { Authorization: 'Bearer foreign-query' },
      });

      expect(onHeadersCaptured).toHaveBeenCalledOnce();
      expect(onHeadersCaptured).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('captures XHR credentials only for exact DeepSeek API origins', () => {
    const nativeXMLHttpRequest = globalThis.XMLHttpRequest;
    const FakeXMLHttpRequest = createFakeXMLHttpRequest('', 204);
    vi.stubGlobal('XMLHttpRequest', FakeXMLHttpRequest);
    let uninstall: (() => void) | null = null;
    ...
    try {
      uninstall = hookXHR();
      const requests = [
        ['https://chat.deepseek.com/api/v0/users/current', 'Bearer deepseek'],
        ['https://evil.example/api/v0/users/current', 'Bearer foreign-path'],
        ['https://evil.example/collect?next=https://chat.deepseek.com/api/v0/users/current', 'Bearer foreign-query'],
      ] as const;
