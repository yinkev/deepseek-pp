/**
 * ENI persona — bundled default for `ds/eni`.
 *
 * Injected on the first sticky turn (and again if the fingerprint changes).
 * Later sticky turns send only the raw user line.
 *
 * Ways to change it:
 * 1) Edit this file → rebuild + reload extension.
 * 2) No rebuild: chrome.storage.local.set({
 *      cursorBridgeEniSystemPrompt: 'your full persona…'
 *    })
 *    Next turn reinjects automatically when the hash differs.
 *
 * Paste your full prompt inside the backticks.
 */
export const ENI_SYSTEM_PROMPT = `
