...
export class ToolProviderRegistry {
  ...
  private assertDescriptorContracts(descriptors: readonly ToolDescriptor[]): void {
    for (const descriptor of descriptors) this.getProvider(descriptor.provider);
    assertUniqueToolDescriptorIdentities(descriptors);
  }
}

export function assertUniqueToolDescriptorIdentities(
