...
export function hookFetch(): () => void {
  ...
  const hookedFetch = async function (this: Window, input: RequestInfo | URL, init?: RequestInit) {
    ...
    if (url !== null && isDeepSeekApiURL(url) && requestHeaders) {
      const captured = captureDeepSeekClientHeaders(requestHeaders);
...
export function hookXHR(): () => void {
  ...
  XMLHttpRequest.prototype.open = function (method: string, url: string | URL, ...rest: any[]) {
    ...
    const previousUrl = xhrUrls.get(this);
    const previousHeaders = xhrHeaders.get(this);
    const routeUrl = typeof url === 'string' ? url : url instanceof URL ? url.href : null;
    let resolvedUrl: string | null = null;
    if (routeUrl !== null) {
      try {
        resolvedUrl = new URL(routeUrl, document.baseURI).href;
      } catch {
        resolvedUrl = null;
      }
    }
    const route = typeof method === 'string' && resolvedUrl !== null
      ? matchDeepSeekWebRoute({ method, url: resolvedUrl })
      : null;
    ...
    if (resolvedUrl === null) xhrUrls.delete(this);
    else xhrUrls.set(this, resolvedUrl);
    xhrHeaders.set(this, {});
    try {
      ...
      if (previousUrl === undefined) xhrUrls.delete(this);
      else xhrUrls.set(this, previousUrl);
  ...
  XMLHttpRequest.prototype.setRequestHeader = function (name: string, value: string) {
    const result = origSetRequestHeader.call(this, name, value);
    const headers = xhrHeaders.get(this);
    if (headers) {
      const existingName = Object.keys(headers).find((key) => key.toLowerCase() === name.toLowerCase());
      if (existingName) headers[existingName] = `${headers[existingName]}, ${value}`;
...
function isDeepSeekApiURL(url: string): boolean {
  try {
    ...
    return parsed.origin === 'https://chat.deepseek.com'
      && (parsed.pathname === '/api/v0' || parsed.pathname.startsWith('/api/v0/'));
  } catch {
    return false;
  }
}
